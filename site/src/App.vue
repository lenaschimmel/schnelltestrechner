...
<v-app id="rapidtest"> <!-- class="hidden"> -->
            ...
            <v-row class="fill-height">
              <v-col cols="12" class="fill-height">
                <v-alert color="red" type="warning" icon="mdi-alert">
                  <p>Dies ist eine sehr frühe Vorschau-Version dieser Seite. Sie wurde bisher nicht von unabhängigen,
                    fachkundigen Menschen auf Plausibilität und Fehlerfreiheit geprüft. Bitte verwende die Ergebnisse
                    mit Vorsicht und Skepsis!</p>
                  <p>Bekannte Fehler, Probleme und Ungenauigkeiten sind in den <a
                      href="https://github.com/lenaschimmel/schnelltestrechner/issues" target="_"
                      class="white--text">GitHub Issues</a> erfasst. Bitte richte weitere Anmerkungen dorthin, oder
                    an die verlinkten <a href="https://twitter.com/Testrechner" target="_"
                      class="white--text">Twitter-</a> und <a href="mailto:mail@schnelltestrechner.de" target="_"
                      class="white--text">Mail</a>-Accounts.</p>
                </v-alert>
                ...
                <v-expansion-panels multiple class="mb-8" v-model="expandedPanelsCalc">
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content color="blue lighten-5" class="pt-4">
                      ...
                      <p>Wie zuverlässig eigentlich ein Schnell- oder Selbst-Test auf Sars-CoV-2 ist, <b>lässt sich
                          unmöglich allgemein beantworten.</b> Neben den Eigenschaften des eigentlichen Tests
                        (<i>Sensitivität</i> und <i>Spezifität</i>) spielt auch die sogenannte
                        <i>Vortestwahrscheinlichkeit</i> eine enorm große
                        Rolle. Und die hängt grundsätzlich von der <i>Inzidenz</i>, sowie dem privatem und beruflichem
                      ...
                      <ul class="mb-4">
                        <li>Die mathematischen Grundprinzipien verstehen, insbesondere das scheinbare paradoxe Phänomen
                          des <a href="https://de.wikipedia.org/wiki/Pr%C3%A4valenzfehler"
                            target="_">Prävalenzfehlers</a>
                        </li>
                      ...
                      <p>Diese Seite möchte bei all diesen Schritten helfen. Der Fokus liegt auf einem Test für eine
                        bestimmte Person zu einem bestimmten Zeitpunkt, also nicht auf Abschätzungen für die
                        Gesamtbevölkerung.
                      </p>
                      <p>Details zur Funktionsweise finden sich im Tab "Weitere Infos".</p>
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content color="blue lighten-5" class="pt-4">
                      ...
                      <template v-if="testsKind == 'list'">
                        <v-container fluid>
                          <v-row no-gutters>
                            <v-col cols="12" sm="6" md="4" no-gutters>
                              <v-checkbox label="Nur Selbst-Tests" v-model="testFilterSelf"></v-checkbox>
                            </v-col>
                            <v-col cols="12" sm="6" md="4" no-gutters>
                              <v-checkbox v-model="testFilterPei">
                                <template v-slot:label>
                                  <div>Nur vom <acronym
                                      title="Paul-Ehrlich-Institut - Bundesinstitut für Impfstoffe und biomedizinische Arzneimittel">PEI</acronym>
                                    evaluiert</div>
                                </template>
                              </v-checkbox>
                            </v-col>
                            <v-col cols="12" sm="6" md="4" no-gutters>
                              <v-checkbox label="Wissenschaftliche Studien" v-model="testFilterStudies"></v-checkbox>
                            </v-col>
                          </v-row>
                        </v-container>

                        <v-text-field solo v-model="testFilter" append-icon="mdi-magnify" label="Suchbegriff"
                          placeholder="Suche nach Hersteller, Name, Distributor, AT-Nummer, Sonderzulassungsnummer">
                        </v-text-field>

                        <v-card>
                          <v-data-table dense v-model="selectedTests" :items="visibleTests" :headers="testHeaders"
                            show-select single-select locale="de-DE" mobile-breakpoint="360"
                              ...
                              <v-tooltip left>
                                <template v-slot:activator="{ on, attrs }">
                                  <div v-bind="attrs" v-on="on">
                                    <v-icon v-for="circle in getDataCircles(item.studies)" :key="circle.color" small
                                      :color="circle.color">
                                      {{ circle.icon }}
                                    </v-icon>
                                  </div>
                            ...
                            <template v-slot:item.attrib="{ item }">
                              <v-tooltip left>
                                <template v-slot:activator="{ on, attrs }">
                                  <div v-bind="attrs" v-on="on">
                                    <v-icon v-for="circle in getAttribCircles(item)" :key="circle.color" small
                                      :color="circle.color">
                                      {{ circle.icon }}
                                    </v-icon>
                                  </div>
                        ...
                        <template
                          v-if="intermediate && this.selectedTest && Object.keys(this.selectedTest.studies).length > 0">
                          <p v-if="Object.keys(this.selectedTest.studies).length > 1" class="mt-4">
                            Quelle der Angaben zu Sensitivität und Spezifität:
                            <v-select v-model="studyId" :items="studies" item-text="author" item-value="id" solo>
                        ...
                        <v-container fluid class="pt-8" v-if="!Number.isNaN(this.sensitivity)">
                          ...
                          <v-row>
                            ...
                            <template v-if="intermediate">
                              <v-col cols="12" sm="6" md="3" no-gutters>Falsch-Negativ-Rate: {{ formatPercent(fnr) }}
                              </v-col>
                              <v-col cols="12" sm="6" md="3" no-gutters>Falsch-Positiv-Rate: {{ formatPercent(fpr) }}
                              </v-col>
                              <v-col cols="12" sm="6" md="3" no-gutters>Bayes-Faktor bei positivem Test: {{
                                formatNumber(bayesFactorPos) }}</v-col>
                              <v-col cols="12" sm="6" md="3" no-gutters>Bayes-Faktor bei negativem Test: {{
                        ...
                        <v-alert icon="mdi-alert" dense outlined type="warning" text class="mt-4"
                          ...
                          Zu diesem Test hat der Hersteller keine Sensitivität und Spezifität angegeben. Bitte einen
                          anderen Test auswählen oder Daten selbst eingeben.
                        </v-alert>

                        <v-alert icon="mdi-alert" dense outlined type="warning" text class="mt-4"
                          v-if="this.selectedTest == null">
                          Bitte einen Test auswählen oder eigene Daten zu Sensitivität und Spezifität eingeben.
                        </v-alert>
                      </template>
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content color="blue lighten-5">
                      ...
                      <template v-if="incidenceSource == 'state'">
                        <p>Bundesland:
                          <v-select v-model="stateId" :items="states" item-text="name" item-value="id" solo>
                          </v-select>
                        </p>
                      </template>
                      ...
                      <template v-if="incidenceSource == 'district'">
                        <p>Stadt / Landkreis:
                          <v-autocomplete v-model="districtId" :items="districtsChoices" item-text="displayName"
                            item-value="ags" solo>
                          </v-autocomplete>
                        </p>
                      </template>
                      ...
                      <template v-if="intermediate">
                        <template v-if="incidenceSource != 'input'">
                          <p><i>Quelle der Inzidenzdaten: <a href="https://api.corona-zahlen.org"
                                target="_">api.corona-zahlen.org</a></i></p>
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content color="blue lighten-5" class="pt-4">
                      ...
                      Zusätzliche Risiken in den letzten 10 Tagen laut <a href="https://www.microcovid.org"
                        target="_">microCOVID.org</a>:
                      <v-text-field v-model="additionalRiskString" background-color="white" suffix="µCoV" solo>
                      </v-text-field>

                      <p v-if="intermediate">Vortestwahrscheinlichkeit vor Symptomen: <input disabled
                          v-bind:value="formatPercent(priorProb)" />
                      </p>
                    </v-expansion-panel-content>
                  </v-expansion-panel>
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content color="blue lighten-5">
                      <v-container fluid>
                        <v-row no-gutters>
                          <v-col cols="12" sm="4" md="4" no-gutters>
                            ...
                            <v-checkbox v-model="sympTaste" label="Geschmacksverlust"></v-checkbox>
                          </v-col>
                          <v-col cols="12" sm="4" md="4" no-gutters>
                          ...
                          <v-col cols="12" sm="4" md="4" no-gutters>
                            <v-checkbox v-model="sympChest" label="Engegefühl in der Brust"></v-checkbox>
                            <v-checkbox v-model="sympWheez" label="Keuchen"></v-checkbox>
                          </v-col>
                          <v-col cols="12" no-gutters>
                            <v-checkbox v-model="sympOther" label="Sonstige verdächtige Symptome" hint="z.B. Schnupfen, Bronchitis, Pneumonie, Atemnot, Halsschmerzen, Kopf-
                      ...
                      <v-alert icon="mdi-information" dense outlined type="info" v-if="sympChest || sympWheez">
                        Hinweis: wenn <i>Keuchen</i> oder <i>Engegefühl in der Brust</i> vorliegen, sinkt die
                        Vortestwahrscheinlichkeit scheinbar. Das verwirrt auf den ersten Blick, hat aber einen Grund,
                        der <a
                          href="https://github.com/lenaschimmel/schnelltestrechner/issues/21#issuecomment-808184992"
                          target="_">hier</a> erläutert wird.
                      </v-alert>
                      ...
                      <p v-if="intermediate">Vortestwahrscheinlichkeit nach Symptomen: <input disabled
                ...
                <v-expansion-panels multiple class="mb-8" v-model="expandedPanelsCompare">
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content color="blue lighten-5">
                      ...
                      <p class="mt-4">Hier kann die Sensititvität und Spezifität von Schnelltests grafisch verglichen
                        werden.</p>

                      <p>Die Filterung funktioniert grundsätzlich wie im Testrechner. Zusätzlich können beliebig viele
                        Tests mit der Checkbox in der linken Spalte für einen engeren Vergleich ausgewählt werden. Wenn
                        anschließend der Filter "Ausgewählte" aktiviert wird, werden nur noch diese angezeigt.
                      </p>

                      <p>
                        Die Grafiken für Sensititvität und Spezifität bilden jeweils einen Bereich von 0% (links) bis
                        100% (rechts) ab.
                        Jede Zeile steht für eine Datenquelle, also die Herstellerangaben oder eine Studie aus der <a
                          href="https://www.klinikum.uni-heidelberg.de/diagnostics-global-health" target="_">Liste
                          wissenschaftlicher Studien</a> der Uni Heidelberg. Kurze, vertikale Linie gibt den Mittelwert
                        an,
                      ...
                      <p>Mit der Maus kann über den einzelnen Zeilen gehovert werden, um die Autor_innen der Studie und
                        die exakten Werte in einem Tooltip anzuzeigen.</p>
                      ...
                      <v-container fluid>
                        <v-row no-gutters>
                          <v-col cols="12" sm="4" md="3" no-gutters>
                            <v-checkbox label="Nur Selbst-Tests" v-model="compareFilterSelf"></v-checkbox>
                          </v-col>
                          ...
                          <v-col cols="12" sm="4" md="3" no-gutters>
                            <v-checkbox label="Wissenschaftliche Studien" v-model="compareFilterStudies"></v-checkbox>
                          </v-col>
                          <v-col cols="12" sm="4" md="3" no-gutters>
                            <v-checkbox label="Ausgewählte" v-model="compareFilterSelected"></v-checkbox>
                          </v-col>
                        </v-row>
                      </v-container>

                      <p>Anzeige-Einstellungen:</p>
                      <v-checkbox label="Spezifität auf oberste 10% heran zoomen" v-model="zoomSpecificity">
                      </v-checkbox>

                      <v-card class="mt-4">
                        <v-data-table dense v-model="compareTests" :items="visibleCompareTests"
                          :headers="compareHeaders" show-select locale="de-DE" mobile-breakpoint="360"
                          :options="{ itemsPerPage: 15 }">
                          <template v-slot:item.sensitivity="{ item }">
                            <svg xmlns="http://www.w3.org/2000/svg" :height="20 * Object.values(item.studies).length"
                              width="201">
                              <rect width="201" :height="20 * Object.values(item.studies).length"
                                style="fill:rgb(255,255,255);stroke-width:1;stroke:rgb(0,0,0)" />
                              <line v-for="x in [10,20,30,40,50,60,70,80,90]" :key="x" :x1="x*2" :x2="x*2" y1="0"
                                :y2="20 * Object.values(item.studies).length" :style="'stroke:grey;stroke-width:1'" />

                              <template v-for="(study,i) in Object.values(item.studies)">
                                <template v-if="study && study.sensitivity">
                                  <rect width="201" :height="20" :y="20 * i" style="fill:transparent;stroke:none"
                                    :key="study.id+'_sp1'">
                                    <title>{{ formatPercent(study.sensitivity.avg) + ' (95% CI: ' +
                                      formatPercent(study.sensitivity.min) + ' bis ' +
                                      formatPercent(study.sensitivity.max) + ') - ' + studyTitle(study) }}</title>
                                  </rect>
                                  <line :key="study.id+'_sp2'" :x1="study.sensitivity.min*200"
                                    :x2="study.sensitivity.max*200" :y1="10 + i * 20" :y2="10 + i * 20"
                                    :style="'stroke:' + getStudyColor(study) + ';stroke-width:2'" />
                                  <line :key="study.id+'_sp3'" :x1="study.sensitivity.avg*200"
                                    :x2="study.sensitivity.avg*200" :y1="5 + i * 20" :y2="15 + i * 20"
                                    :style="'stroke:' + getStudyColor(study) + ';stroke-width:2'" />
                                </template>
                              </template>
                          ...
                          <template v-slot:item.specificity="{ item }">
                            <svg xmlns="http://www.w3.org/2000/svg" :height="20 * Object.values(item.studies).length"
                              ...
                              <g v-if="!zoomSpecificity">
                                <line v-for="x in [10,20,30,40,50,60,70,80,90]" :key="x" :x1="x*2"
                                  :x2="x*2" y1="0" :y2="20 * Object.values(item.studies).length"
                              ...
                              <template v-for="(study,i) in Object.values(item.studies)">
                                <template v-if="study && study.specificity">
                                  <rect width="201" :height="20" :y="20 * i" style="fill:transparent;stroke:none"
                                    :key="study.id+'_sp1'">
                                    <title>{{ formatPercent(study.specificity.avg) + ' (95% CI: ' +
                                      formatPercent(study.specificity.min) + ' bis ' +
                                      formatPercent(study.specificity.max) + ') - ' + study.author + ', n = ' +
                                      (study.sampleSize || 'unbekannt') }}</title>
                                  </rect>
                                  <line :key="study.id+'_sp2'" :x1="getSpecificityX(study.specificity.min)"
                                    :x2="getSpecificityX(study.specificity.max)" :y1="10 + i * 20" :y2="10 + i * 20"
                                    :style="'stroke:' + getStudyColor(study) + ';stroke-width:2'" />
                                  <line :key="study.id+'_sp3'" :x1="getSpecificityX(study.specificity.avg)"
                                    :x2="getSpecificityX(study.specificity.avg)" :y1="5 + i * 20" :y2="15 + i * 20"
                                    :style="'stroke:' + getStudyColor(study) + ';stroke-width:2'" />
                                </template>
                              </template>
                ...
                <v-expansion-panels multiple class="mb-8" v-model="expandedPanelsInfo">
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content color="blue lighten-5">
                      ...
                      <p class="font-weight-bold mt-4">Die Daten über verfügbare Schnelltests kommen aus diesen Quellen:
                      </p>
                      <ul>
                        <li><a href="https://www.bfarm.de/DE/Medizinprodukte/Antigentests/_node.html" target="_">Liste
                            der Tests zur Eigenanwendung durch Laien</a> des BfArM</li>
                        <li><a href="https://antigentest.bfarm.de/ords/f?p=ANTIGENTESTS-AUF-SARS-COV-2" target="_">Liste
                            der Antigen-Tests zum direkten Erregernachweis des Coronavirus
                            SARS-CoV-2</a> des BfArM</li>
                        <li><a href="https://www.klinikum.uni-heidelberg.de/diagnostics-global-health"
                            target="_">Ergebnisse wissenschaftlicher Studien</a>, zusammen getragen von der Uni
                          Heidelberg
                        </li>
                      </ul>
                      <p class="font-weight-bold mt-4">Die Einschätzung der a-priori Wahrscheinlichkeit basiert auf:</p>
                      <ul>
                        <li>den Inzidenzen gemäß <a href="https://api.corona-zahlen.org/docs/"
                            target="_">corona-zahlen.org</a></li>
                        <li>einer Extrapolation der Inzidenz durch das Wachstum seit der Vor-Woche</li>
                        <li>einer Abschätzung des Underreporting-Faktors von 4-6 durch das <a
                            href="https://www.rki.de/DE/Content/Infekt/EpidBull/Archiv/2020/Ausgaben/50_20.pdf?__blob=publicationFile"
                            target="_">RKI</a></li>
                        <li>Faktoren für das private Infektionsrisiko, die an <a href="https://www.microcovid.org"
                            target="_">Microcovid</a> angelehnt sind</li>
                        <li>Faktoren für das berufliche Risiko, die beispielhaft aus Grafiken von <a
                            href="https://www.addendum.org/coronavirus/berufe-ansteckungsrisiko/"
                            target="_">www.addendum.org</a> entnommen wurden</li>
                      </ul>
                      <p class="mt-4">Die Berechnung der Bayes-Faktoren und der A-Posteriori-Wahrscheinlichkeiten folgt
                        der
                        Berechnung des Videos <a href="https://www.youtube.com/watch?v=lG4VkPoG3ko" target="_">The
                          medical test paradox: Can redesigning Bayes rule help?</a> von 3Blue1Brown.
                      </p>
                      <p>Der Einfluss von Symptomen auf die a-priori Wahrscheinlichkeit einer Infektion wurde nach der
                        <a href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7584484/" target="_">Punkteregel nach
                          Smith, Richey und Brunetto</a> implementiert. Die Punkteregel ist darauf ausgelegt, die
                        a-priori Wahrscheinlichkeit eines symptomatischen Patienten (mit unklaren Symptomen, die grob
                        auf
                        Covid-19 hindeuten) anhand seiner spezifischen Symptome zu verfeinern. Dazu wird die Prävalenz
                        um den Faktor 18,8 angehoben, sobald Symptome vorhanden sind, um anschließend die Punkteregel
                        anzuwenden. <strong>Der Faktor 18,8 basiert auf sehr groben Abschätzungen und stellt
                          wahrscheinlich die größte Ungenauigkeit innerhalb der gesamten Berechnung dar. Er sollte
                          dringend durch einen wissenschaftlich basierten Wert ersetzt werden.</strong>
                      </p>
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content color="blue lighten-5">
                      ...
                      <p class="mt-4">Diese Seite hat nicht den Anspruch, sämtliche Informationen zu SARS-CoV-2-Tests
                        vollständig und
                        fehlerfrei anzubieten. Daher empfehlen wir ausdrücklich, auch weitere Informationsquellen zu
                        Rate zu ziehen!</p>
                      <ul>
                        <li><a href="https://www.zusammengegencorona.de/informieren/testen/" target="_">Informationen
                            zum Testen</a> vom Bundesministerium für Gesundheit</li>
                        <li><a href="https://www.infektionsschutz.de/coronavirus/tests-auf-sars-cov-2.html"
                            target="_">Tests auf das Coronavirus SARS-CoV-2</a> von der Bundeszentrale für
                          gesundheitliche Aufklärung (BZgA)</li>
                        <li>Sämtliche Informationen des <a
                            href="https://www.rki.de/DE/Content/InfAZ/N/Neuartiges_Coronavirus/nCoV.html"
                            target="_">Robert Koch Instituts (RKI)</a></li>

                        <li>Der <a href="https://diagnostics-global-health.github.io/rechner/" target="_">
                            Schnelltest-Rechner der Uni-Heidelberg</a>, von der auch die Studiendaten kommen. Schnell
                          und
                          einfach, aber ohne individuelle Vortest-Wahrscheinlichkeit.</li>
                        <li>Der <a href="https://rki-wiko.shinyapps.io/test_qual/" target="_"> Schnelltest-Rechner des
                            Robert Koch Instituts (RKI)</a>. Er ist zwar langsam und weniger mächtig als dieser hier,
                          bietet aber ein paar andere Darstellungsweisen der Ergebnisse.</li>
                        <li>Im <a
                            href="https://www.ndr.de/nachrichten/info/Das-Glossar-zum-Corona-Podcast,podcastcoronavirus146.html#SARS"
                            target="_">Corona-Glossar</a> des <a
                            href="https://www.ndr.de/nachrichten/info/podcast4684.html">Coronavirus-Update von NDR
                            Info</a> werden alle wichtigen Fachbegriffe erläutert. Generell ist der Podcast sehr
                          empfehlenswert und es gibt ein <a
                            href="https://www.ndr.de/nachrichten/info/coronaskript174.pdf" target="_">durchsuchbares
                            Transkript aller Folgen als PDF</a>.</li>
                        <li>Die Initiative <a href="https://rapidtests.de/" target="_">RapidTests.de</a> erläutert
                          wichtige Konzepte, auch in ihren umfangreichen <a href="https://rapidtests.de/faq"
                            target="_">FAQ</a>.</li>
                        <li>Die <a
                            ...
                            target="_">Zusammenfassung</a> und die <a
                            href="https://www.cochranelibrary.com/cdsr/doi/10.1002/14651858.CD013705.pub2/full/de"
                            target="_">(wirklich lange) Langfassung</a> des Aktualisierten Cochrane Review bewertet
                          Zuverlässigkeit von Schnelltests zum Nachweis von COVID-19. Die Ergebnisse daraus sind <i>noch
                            nicht</i> in die Datenbasis dieses Rechners integriert.</li>
                      </ul>
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content color="blue lighten-5">
                      ...
                      <p class="mt-4">In der Auswahlliste sind Test gekenntzeichnet, die durch das Paul-Ehrlich-Institut
                        (PEI) evaluiert sind. Auch lässt sich die Liste nach diesem Kriterium filtern.</p>
                      <p><b>Die Relevanz der PEI-Evaluation kann leicht überschätzt werden, da der Anschein entsteht,
                          die
                          Tests würden unabhängig bzgl. Sensititvität <i>und</i> Spezifität getestet werden. Das ist
                          nicht der
                          Fall.</b></p>
                      <p>Das PEI fragt u. A. die Sensitivität und
                        Spezifität vom Hersteller ab, und legt daran Mindestkriterien an. Nur wenn ein Test laut
                        Hersteller mindestens 80% Sensititvität und mindestens 97% Spezifität hat, kommt er für die
                        Evaluierung überhaupt in Frage.</p>
                      <p>Die Tests, die nicht bereits durch die Hersteller-Angaben ausscheiden, <i>können</i> dann vom
                        PEI weiter betrachtet werden, worauf jedoch kein Anspruch besteht.</p>
                      <p>Für die erfolgreiche Evaluierung ist ebenfalls eine Laboruntersuchung durch <i>"verschiedene
                          Institutionen in Deutschland (u.a. Robert-Koch-Institut; Paul-Ehrlich-Institut; Konsiliarlabor
                          für Coronaviren; Institut für Mikrobiologie der Bundeswehr)"</i> nötig. In einem
                        einheitlichen, analytischen (praxis-fernen, aber sinnvollem) Verfahren wird die Sensitivität des
                        Tests geprüft. <b>Der
                          Labortest im Rahmen der PEI-Evaluation testet <i>nicht</i> die Spezifität.</b></p>
                      <p>Der durchgeführte Test ist zwar geeignet, eine vage Aussage zur Sensititvität zu machen, jedoch
                        wird dabei kein Wert Ermittelt, der direkt mit den Ergebnissen der Hersteller oder anderer
                        unabhängiger Studien verglichen werden kann. Insbesondere wird vom PEI kein solcher Wert als
                        Prozentangabe veröffentlicht, sondern nur die Liste der Tests, die als <i>"dem derzeitigen Stand
                          der Technik entsprechend bewertet wurden"</i>.</p>
                      ...
                      <ul>
                        <li><a
                            ...
                            target="_">Mindestkriterien für SARS-CoV-2 Antigentests im Sinne von § 1 Abs. 1 Satz 1
                            TestVO: Antigenschnelltests</a></li>
                        <li><a
                            ...
                            target="_">Vergleichende Evaluierung der Sensitivität von SARS-CoV-2 Antigenschnelltests
                          </a></li>
                      </ul>
                    </v-expansion-panel-content>
                  </v-expansion-panel>
              ...
              <v-col cols="12" class="fill-height">
                <v-expansion-panels multiple class="mb-8" v-model="expandedPanelsImprint">
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content>
                      <h2>Angaben gemäß § 5 TMG und Verantwortliche für den Inhalt nach § 55 Abs. 2 RStV
                      </h2>
                      <ul>
                        <li>Lena Schimmel</li>
                        <li>Chemnitzstr. 7</li>
                        <li>38118 Braunschweig</li>
                        <li>E-Mail: <a href="mailto:mail@schnelltestrechner.de">mail@schnelltestrechner.de</a></li>
                      </ul>
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content>
                      <p>Bei der Nutzung dieser Webseite werden keine personenbezogenen Daten, insbesondere keine
                        Gesundheitsdaten erhoben, da alle Eingaben lokal im Browser verarbeitet werden. Es werden keine
                        Cookies o.Ä. verwendet, und keine Anfragen an andere Anbieter und Domains gemacht.</p>
                    </v-expansion-panel-content>
                  </v-expansion-panel>
                  ...
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content>
                      <p>Mein Angebot enthält Links zu externen Webseiten Dritter, auf deren Inhalte wir keinen Einfluss
    ...
    <v-footer height="70" v-if="tab == 0 && !resultValid" :app="fixedFooter" :fixed="fixedFooter" color="red lighten-3"
      id="footer">
      <v-card-text>Ergebnis kann nicht ermittelt werden, bevor die Eingaben vollständig sind.</v-card-text>
    </v-footer>

    <v-footer :height="$vuetify.breakpoint.xsOnly ? 250 : 200" v-if="tab == 0 && resultValid" :app="fixedFooter" :fixed="fixedFooter"
      color="light-blue lighten-3" id="footer">
      <v-container fluid>
        <v-row v-if="$vuetify.breakpoint.xsOnly">
          <v-col class="font-weight-bold text-center" cols="12">Vortestwahrscheinlichkeit {{
            formatPercent(priorProbSymptoms) }}
          </v-col>
        </v-row>
        <v-row>
          <v-col cols="12" sm="4" md="3" class="font-weight-bold mt-3" v-if="$vuetify.breakpoint.smAndUp">
            Vortestwahrscheinlichkeit:
    ...
    <v-fab-transition v-if="tab == 0">
      <v-btn color="pink" v-show="!fixedFooter && !scrolledToBottom" fab dark fixed bottom right class="mb-8 mr-8"
